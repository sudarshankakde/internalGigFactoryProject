...
import { X, AlertTriangle, UploadCloud, FileText } from "lucide-react";
...
export default function EditPaymentModal({
  ...
  const [paymentAmount, setPaymentAmount] = useState(
    editingPayment.amount ? editingPayment.amount.toString() : "",
  );
  const [paymentMethod, setPaymentMethod] = useState(
    editingPayment.payment_method || "bank_transfer",
  );
  const [transactionRef, setTransactionRef] = useState(
    editingPayment.transaction_reference || "",
  );
  const [paymentRemarks, setPaymentRemarks] = useState(
    editingPayment.remarks || "",
  );
  ...
  const handleEditPaymentSubmit = async (e) => {
    ...
    try {
      await api.putFile(`/projects/payments/${editingPayment.id}`, formData);
      toast.success("Milestone payment updated successfully!");
      onSuccess();
    } catch (err) {
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#23232a] bg-[#0c0c0e] shrink-0">
          <div>
            ...
            <p className="text-gray-500 text-[0.75rem] m-0 mt-1">
              Update payment release and transaction details.
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              {proofFile ? (
                ...
                <>
                  ...
                  <span className="text-gray-400 text-[0.8rem] font-medium group-hover:text-gray-200 transition-colors">
                    Select new receipt or PDF proof (optional update)
                  </span>
